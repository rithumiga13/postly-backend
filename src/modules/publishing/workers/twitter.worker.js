...
export function createTwitterWorker() {
  const worker = new Worker(queueName('twitter'), processJob, {
    connection: getConnectionOpts(),
    concurrency: 5,
  });

  worker.on('completed', (job) => {
    logger.info({ jobId: job.id }, 'twitter job completed');
  });

  worker.on('failed', async (job, err) => {
    logger.error({ jobId: job?.id, attemptsMade: job?.attemptsMade, err }, 'twitter job failed');
  ...
  worker.on('error', (err) => {
    logger.error({ err }, 'twitter worker error');
  });

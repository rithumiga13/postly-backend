...
export function createThreadsWorker() {
  const worker = new Worker(queueName('threads'), processJob, {
    connection: getConnectionOpts(),
    concurrency: 5,
  });

  worker.on('completed', (job) => {
    logger.info({ jobId: job.id }, 'threads job completed');
  });

  worker.on('failed', async (job, err) => {
    logger.error({ jobId: job?.id, attemptsMade: job?.attemptsMade, err }, 'threads job failed');
  ...
  worker.on('error', (err) => {
    logger.error({ err }, 'threads worker error');
  });

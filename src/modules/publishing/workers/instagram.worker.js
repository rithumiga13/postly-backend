...
export function createInstagramWorker() {
  const worker = new Worker(queueName('instagram'), processJob, {
    connection: getConnectionOpts(),
    concurrency: 5,
  });

  worker.on('completed', (job) => {
    logger.info({ jobId: job.id }, 'instagram job completed');
  });
  ...
  worker.on('error', (err) => {
    logger.error({ err }, 'instagram worker error');
  });
